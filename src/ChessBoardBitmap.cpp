...
ChessBoardBitmap::ChessBoardBitmap( int pix_ )
{
    pickup_file    = 0;
    pickup_rank    = 0;
    pickup_point.x = 0;
...
{
	// NEXT STEP - target is 364 wide, 294 high
	//  Create miniboard as below, then target, then methods to copy rect->rect and use them to build up target
	int pix_ = 34;
	wxSize size( pix_*8, pix_*8 );
	current_size = size;
...
void Testbed()
{
	ChessBoardBitmap cbm(50);
    wxBitmap bm( board_setup_bitmap_xpm );
	TestbedDialog dialog(cbm.my_chess_bmp);
	dialog.Run();
}